...
class ApiController extends Controller
{
    ...
    {
        $childList = User::where('parent_id', $request->user()->id)->get();
        return response()->json(array('children' => $childList));
    }


    public function getReports(Request $request, $child): JsonResponse
    {
        $parentId = $request->user()->id;
        $child = User::where('parent_id', $parentId)->where("id", $child)->first();
        ...
        $pythonCorrectCount = $this->getTotalCorrect($language, $child);
        $rubyCorrectCount = $this->getTotalCorrect($language2, $child);
        $scratchCorrectCount = $this->getTotalCorrect($language3, $child);
        ...
        return response()->json(
            array(
                ...
                'data' => [
                    ...
                    'pythonCorrectCount' => $pythonCorrectCount,
                    'rubyCorrectCount' => $rubyCorrectCount,
                    'scratchCorrectCount' => $scratchCorrectCount,
                ]
            )
        );
    }
...
//    }
    ...
    private function getTotalAnswers($language, $child): int
    {
    ...
    private function getTotalCorrect($language, $child): int
    {
        return DB::table('progress')
            ->join('questions', 'progress.question_id', '=', 'questions.id')
            ->join('topics', 'questions.topic_id', '=', 'topics.id')
            ->join('courses', 'topics.course_id', '=', 'courses.id')
            ->where('progress.user_id', $child->id)
            ->where('progress.select_answer', DB::raw('questions.correct_answer'))
            ->where('courses.language_id', $language->id)
            ->count();
    }
}
